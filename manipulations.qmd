...
The `ifelse()` function is a vectorized conditional function handy for creating indicator, ordinal, or categorical variables. It has the form `ifelse(test, yes, no)` , where `test` is a logical condition, `yes` is the value to return for observations where the `test` is `TRUE`, and `no` is the value to return where the `test` is
`FALSE`. The value returned by `ifelse()` will have the same length as the `test` vector, with each element filled by either the `yes` or `no` value depending on the condition[@matloff2011art].
...
We can also nest `ifelse()` statements to create variables with more than two categories. Continuing the example above, suppose we want to create `School3` such that: A/B/C map to 1, D/E map to 2, and F/G/H
map to 3. We can nest a second `ifelse()` inside the first:
...
Data often contain duplicate records, which may occur intentionally or as errors (for instance, through data entry mistakes or merging datasets). Identifying and handling duplicates is typically part of data cleaning.
Base R provides functions to detect and manage duplicates in vectors, matrices, and data frames.

### Duplicated Elements in Vectors

For vectors, the `unique()` function returns a vector of the distinct values. The companion function `duplicated()` returns a logical vector the same length as the input, indicating for each position whether
that element is a duplicate of an earlier element. Both functions have an argument `fromLast` which, if set to `TRUE`, considers duplication from the reverse direction (this affects which element is considered the
“first” occurrence).
...
Here `unique(v)` kept the first occurrence of each letter (so we see `"a"` from position 1, not the `"a"` from the end), whereas `unique(v, fromLast=TRUE)` kept the last occurrence of each duplicate (so it
retained the `"a"` from the end and dropped the earlier one).
...
If we want to check for duplicated columns in a matrix, we can use the `MARGIN` argument with value 2 in `unique()` or `duplicated()` . By default, these functions act on rows (`MARGIN=1`). For example, `unique(mat, MARGIN=2)` would give the unique set of columns of the matrix, and `duplicated(mat,
MARGIN=2)` would flag duplicate columns. In our mat example, we can test:
...
Merging them by `id` will now result in a data frame that includes only the rows with matching `id` values in both data frames.
...
By default, `merge()` performs an inner join (keeping only matching rows). We can change this behavior with the arguments `all.x` , `all.y` , or `all` :
...
Merging data frames in base R using `merge()` is quite powerful for relational operations. It’s worth noting, however, that there are other packages and tools (such as `dplyr` from the `tidyverse`, or the `data.table` package) that provide alternative syntax and potentially faster performance for merging and
joining operations. 
...
For more complex data rearrangements, base R provides the `reshape()` function. The `reshape()` function is more general than `stack()` / `unstack()` and can handle conversions of data frames from long format to wide format and vice versa, even in cases where `stack()` / `unstack()` are not sufficient (for example, when there are multiple measured variables or an unbalanced structure). 

The `reshape()` function in base R works by specifying certain key arguments:

-   `data` : the data frame to reshape.
-   `direction` : `"wide"` or `"long"` indicating which way you want to reshape the data.

For reshaping to wide, you’ll specify:

-   `idvar` : one or more columns that identify the subject or observational unit (that stay the same for all reshaped rows).
-   `timevar` : the name of the column that indicates the time or occasion (whose values will become suffixes in wide format).

For reshaping to long, you’ll need to specify the opposite: which columns in wide form should be gathered into a single column, and how to name the resulting long-form identifiers.

-   `varying` : a key argument in `reshape()` which specifies the names of multiple columns that correspond to the same variable measured at different times. It can be a list of vectors of names (or indices) of the columns that go together.


The usage can be a bit complex at first. Let’s illustrate with an example. Suppose we have a data frame in *wide* format for an experiment where each subject (`id`) is measured at two time points on a variable
`score` . Our wide data might look like:
...
Here each row is a subject, and the measurements at time1 and time2 are in separate columns. To reshape this to long format (one row per subject per time), we can use r`eshape()`:

```{r}
longDF <- reshape(wideDF, 
                  direction = "long",
                  idvar = "id", 
                  timevar = "time",
...
-   `idvar = "id"` tells `reshape()` that `“id”` identifies a subject (stays constant across reshaped rows belonging to the same subject). 
-   `timevar = "time"` means we will create a new column named `time` to indicate the time point. 
-   `varying = c("score_time1","score_time2")` tells which columns in wide form contain the measurements that need to be stacked. Here we have two such columns.
-   `v.names = "score"` sets the name of the measurement variable in the long form.

In the result, each subject now has two rows: time = 1 and time = 2, with their corresponding scores. By default, `reshape()` uses the numeric suffixes from `score_time1` and `score_time2` to determine the values of the new time variable. If those suffixes were non-numeric or ambiguous, we could use the times argument (e.g., `times = c(1, 2)`) to explicitly define the time values.

Now, `reshape()` can go the other way as well. If we start with a long dataset, we can cast it to wide by specifying `direction = "wide"` and appropriate id and time variables. For example, we can reshape `longDF` back to wide format as follows:

```{r}
wideDF2 <- reshape(longDF, 
                   direction = "wide", 
                   idvar = "id", 
                   timevar = "time")
wideDF2
...
The names `score.1` and `score.2` are the default naming convention indicating `score` at time 1 and time 2. We could change the separator by specifying the `sep` argument.

The base R `reshape()` function is powerful but can be confusing. It requires carefully specifying the identifiers and varying columns. Modern packages (like `reshape2` or `tidyr`) provide simpler functions (`melt()`, `dcast()` or `pivot_longer()`, `pivot_wider()`) which are often easier to use for these tasks. 
...
Before diving into each function, note that all these functions ultimately achieve a “split-apply-combine” strategy: they split data into groups based on some factor or categorical variable, apply a function to each
group, and then combine the results.

### Aggregation with `aggregate()`

The `aggregate()` function is a data-frame-oriented alternative to `tapply()`. It can handle multiple columns at once and returns a data frame as output, making it convenient for further data manipulation or merging. In contrast, `tapply()` is typically used for applying a function to a numeric vector split by one or more factors, and it returns an array or vector, depending on the input.

The basic usage is `aggregate(x, by, FUN)` where: 

-   `x` is the data to aggregate. This can be a vector or a data frame 
-   `by` is a list of grouping variables or a formula specifying the grouping. -    `FUN` is the function to apply.

Consider the built-in `iris` dataset, which contains the measurements in centimeters of the variables sepal length and width and petal length and width, respectively, for 50 flowers from each of 3 species of iris.
...
For example, using the `iris` data, we can get the mean of sepal length by species, by specifying `Sepal.Length ~ Species`, in aggregate():

```{r}
aggregate(Sepal.Length ~ Species, data = iris, FUN = mean)
```

Here aggregate has created a data frame with one row per Species and a column for the mean `Sepal.Length`.

We can also aggregate multiple measurements simultaneously. For instance, to compute the mean and standard deviation of both `Sepal.Length` and `Sepal.Width` by species, we use `cbind()` on the left side of the tilde (`~`) to combine the variables, and apply a user-defined function:

```{r}
aggregate(cbind(Sepal.Length, Sepal.Width) ~ Species, iris, 
          \(x){
...
If we want to aggregate by multiple factors (say we had another factor like `Species` and `SomeFactor`), we could write:

```{r, eval=FALSE}
aggregate(Sepal.Length ~ Species + SomeFactor, data = iris, FUN = mean)
```

`aggregate()` has a formula method and a default method. You can also call it like:

```{r}
aggregate(x = iris[, 1:4], by = list(Species = iris$Species), FUN = mean)
```

Here we explicitly pass a data frame of the first four columns of iris as `x`, and a list containing the grouping factor (with a name `"Species"`). 
...
```{r}
students$class_avg <- ave(students$score, students$class, FUN=mean)
students
```
...
Although the default function for `ave()` is mean, you can supply other summary functions like `median`, `sd`, or even custom ones as long as the function returns a single value per group. However, avoid using functions that return multiple values (like range), since a`ve()` expects only one summary value per group.
